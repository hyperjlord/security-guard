...
public class BoundBox {
    ...
    public BoundBox(double north, double south, double east, double west, int radius) {
        ...
        //确定包围盒宽高
        width = ((int)((Operation.sub(east,west))*Math.pow(10,5))+1) + 2*radius;
        height = ((int)((Operation.sub(north,south))*Math.pow(10,5))+1) + 2*radius;
        bound_box = new short[height][width];
    }

    //将经纬度转化为包围盒的坐标
    public int[] transform(double lat, double lng, int radius) {
        int x = (int)(Operation.sub(lat, south_lat)*Math.pow(10,5)) + radius;
        int y = (int)(Operation.sub(lng, west_lng)*Math.pow(10,5)) + radius;
    ...
    public double[] transform_reverse(int x, int y, int radius) {
        double lat = Operation.sum((x-radius)*Math.pow(10,-5), south_lat);
        double lng = Operation.sum(((y-radius)*Math.pow(10,-5)), west_lng);
    ...
    //根据本周路径修改包围盒中各坐标值参数
    public void trackValues(Boundary boundary, RouteSet route_set, int radius, double U, short initial_prob, double k) {
        /**
         * boundary: 边界
         * route_set: 路径集合
         * radius: 权值扩散半径
    ...
    //收割最后的边界
    public void reap(Boundary boundary, short min, int radius) {
        /**
         * boundary: 边界
         * min: 最低阈值
         * radius: 权值扩散半径*/
        for (int i = 0; i < height/2; i++) {
            for (int j = 0; j < width/2; j++) {
                ...
                if (bound_box[i][j] < min) {
                    ...
                    bound_box[i][j] = bound_box[i][j]>100?100:bound_box[i][j];
                    if(boundary.isInBound(location[0],location[1])) {
